...
func (l *DelFocusLogic) DelFocus(req types.DelFocusReq) (resp *types.DelFocusResp, err error) {
    ...
    //增加关注关系
    _, err = l.svcCtx.UserBehaviorRpc.DelFocus(l.ctx, &rpcuserbehavior.DelFocusReq{
        AppId:    req.AppId,
        FocusUin: req.FocusUin,
    })
    if err != nil {
        filed := map[string]interface{}{"sender": "USER-BEHAVIOR-API", "code": 0, "uin": "", "req": reqStr, "resp": "", "track_data": ""}
        l.svcCtx.MyLogger.WithFields(mylogrus.GetCommonField(l.ctx, filed)).Error("取消关注失败")
        return nil, errorx.NewDefaultError("取消关注失败")
    }
    ...
    //if err != nil {
    //    filed := map[string]interface{}{"sender": "USER-BEHAVIOR-API", "code": 0, "uin": "", "req": reqStr, "resp": "", "track_data": ""}
    //    l.svcCtx.MyLogger.WithFields(mylogrus.GetCommonField(l.ctx, filed)).Error("取消关注计数失败")
    //}
    //
    ////调用粉丝计数方法
