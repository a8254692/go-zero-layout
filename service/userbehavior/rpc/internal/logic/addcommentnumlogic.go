...
type AddCommentNumLogic struct {
    ctx    context.Context
    svcCtx *svc.ServiceContext
    logx.Logger
}

func NewAddCommentNumLogic(ctx context.Context, svcCtx *svc.ServiceContext) *AddCommentNumLogic {
    return &AddCommentNumLogic{
        ctx:    ctx,
        svcCtx: svcCtx,
...
// 新增评论计数
func (l *AddCommentNumLogic) AddCommentNum(in *userBehaviorProto.AddCommentNumReq) (*userBehaviorProto.AddCommentNumResp, error) {
    ...
    //先查数据库中是否存在数据
    existsRsV := l.svcCtx.Redis.HExists(userCommentNumKey, field).Val()
    if !existsRsV {
        dbInfo, err := l.svcCtx.ProduceCountModel.FindOneByParam(int64(in.TopicType), in.TopicId)
        if err != nil {
            filed := map[string]interface{}{"sender": "USER-BEHAVIOR-RPC", "code": 0, "uin": "", "req": reqStr, "resp": "", "track_data": ""}
            l.svcCtx.MyLogger.WithFields(mylogrus.GetCommonField(l.ctx, filed)).Error("获取DB评论计数失败")
        } else {
            if dbInfo.CommentNum > 0 {
                err = l.svcCtx.Redis.HSet(userCommentNumKey, field, dbInfo.CommentNum).Err()
                if err != nil {
                    filed := map[string]interface{}{"sender": "USER-BEHAVIOR-RPC", "code": 0, "uin": "", "req": reqStr, "resp": "", "track_data": ""}
                    l.svcCtx.MyLogger.WithFields(mylogrus.GetCommonField(l.ctx, filed)).Error("设置DB评论计数失败")
                }
            }
        }
    }
    ...
    err := l.svcCtx.Redis.HIncrBy(userCommentNumKey, field, int64(incr)).Err()
    if err != nil {
        filed := map[string]interface{}{"sender": "USER-BEHAVIOR-RPC", "code": 0, "uin": "", "req": reqStr, "resp": "", "track_data": ""}
        l.svcCtx.MyLogger.WithFields(mylogrus.GetCommonField(l.ctx, filed)).Error("新增评论计数失败")
        return nil, errors.New("新增评论数量失败")
    }

    l.svcCtx.Redis.Expire(userCommentNumKey, help.GetTodayTimeRemaining())

    return &userBehaviorProto.AddCommentNumResp{}, nil
}
